...
`Preezid` 默认提供了一个存储在 `localStorage` 中的简单持久化存储方案.

与 `store()` 类似, `local()` 会返回包括 `.get()`, `.set()` 方法的对象.

当声明 `local()` 时, 需要传入一个键名, 用于在 `localStorage` 中存储数据, 同时也可以链式调用 `.default()` 方法来设置默认值.

```lts
define {
  const themeStore = local<"light" | "dark">("theme").default("light")
  // 若 localStorage 中无 "theme" 键, 则使用默认值
  // 此外, 如果无论 localStorage 中是否存在该键, 都想重置为新值, 则使用 .init() 的方式
  // const themeStore = local<"light" | "dark">("theme").init("light")
