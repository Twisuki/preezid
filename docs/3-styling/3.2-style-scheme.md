...
对于复杂节点树, 每个 `style module` 内部命名作用域局部, 外部命名作用域为整个文件, 例如如下较复杂的例子:

```lts
load {
  div.style(S.section) {
    div.style("title") { "这是标题" }
    div.style("content") {
      features.forEach(item => {
        div
          ...
          .style(S.card)
          {
            ...
            div.style("content") {
              div.style("title") { item.title }
              div.style("description") { item.description }
            }
          }
      })
    }
